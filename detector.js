...
const benchmarkEnabled =
  typeof process !== 'undefined' ? !!process.env.DETECTOR_BENCHMARK : false
...
const Detector = {
  ...
  getTechnology: (name) =>
    [
      ...
      ...Detector.requires.map(({ technologies }) => technologies).flat(),
      ...Detector.categoryRequires
        .map(({ technologies }) => technologies)
        .flat(),
    ].find(({ name: _name }) => name === _name),

  getCategory: (id) => Detector.categories.find(({ id: _id }) => id === _id),
  ...
  resolve(detections = []) {
    ...
    Detector.resolveExcludes(resolved)
    Detector.resolveImplies(resolved)

    const priority = ({ technology: { categories } }) =>
      categories.reduce(
        (max, id) => Math.max(max, Detector.getCategory(id).priority),
        0
      )
    ...
    return resolved
      ...
      .map(
        ({
          ...
          description,
          slug,
          categories: categories.map((id) => Detector.getCategory(id)),
          confidence,
          version,
          icon,
          website,
  ...
  resolveExcludes(resolved) {
    resolved.forEach(({ technology }) => {
      technology.excludes.forEach(({ name }) => {
        const excluded = Detector.getTechnology(name)
      ...
      resolved.forEach(({ technology, confidence, lastUrl }) => {
        technology.implies.forEach(
          ({ name, confidence: _confidence, version }) => {
            const implied = Detector.getTechnology(name)
   ...
   * @param {*} param0
   */
  analyze(items, technologies = Detector.technologies) {
    benchmarks = []

    const oo = Detector.analyzeOneToOne
    const om = Detector.analyzeOneToMany
    const mm = Detector.analyzeManyToMany
  ...
  setTechnologies(data) {
    const transform = Detector.transformPatterns

    Detector.technologies = Object.keys(data).reduce((technologies, name) => {
      const {
        cats,
        certIssuer,
        cookies,
    ...
    Detector.technologies
      ...
      .forEach((technology) =>
        technology.requires.forEach(({ name }) => {
          if (!Detector.getTechnology(name)) {
            throw new Error(`Required technology does not exist: ${name}`)
          }

          Detector.requires[name] = Detector.requires[name] || []

          Detector.requires[name].push(technology)
        })
      )

    Detector.requires = Object.keys(Detector.requires).map((name) => ({
      name,
      technologies: Detector.requires[name],
    }))
    ...
    Detector.technologies
      ...
      .forEach((technology) =>
        technology.requiresCategory.forEach(({ id }) => {
          Detector.categoryRequires[id] =
              Detector.categoryRequires[id] || []

          Detector.categoryRequires[id].push(technology)
        })
      )

    Detector.categoryRequires = Object.keys(Detector.categoryRequires).map(
      (id) => ({
        categoryId: parseInt(id, 10),
        technologies: Detector.categoryRequires[id],
      })
    )

    Detector.technologies = Detector.technologies.filter(
   ...
   */
  setCategories(data) {
    Detector.categories = Object.keys(data)
      .reduce((categories, id) => {
        const category = data[id]

        categories.push({
          id: parseInt(id, 10),
          slug: Detector.slugify(category.name),
          ...category,
        })
  ...
  transformPatterns(patterns, caseSensitive = false, isRegex = true) {
    ...
    const parsed = Object.keys(patterns).reduce((parsed, key) => {
      parsed[caseSensitive ? key : key.toLowerCase()] = toArray(
        patterns[key]
      ).map((pattern) => Detector.parsePattern(pattern, isRegex))
  ...
  parsePattern(pattern, isRegex = true) {
    if (typeof pattern === 'object') {
      return Object.keys(pattern).reduce(
        (parsed, key) => ({
          ...parsed,
          [key]: Detector.parsePattern(pattern[key]),
        }),
        {}
      )
    } else {
    ...
    return technology[type].reduce((technologies, pattern) => {
      ...
      if (matches) {
        technologies.push({
          ...
          version: Detector.resolveVersion(pattern, value),
        })
      }
