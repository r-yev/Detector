...
function getJs(page, technologies = Detector.technologies) {
...
function analyzeJs(js, technologies = Detector.technologies) {
  return js
...
function getDom(page, technologies = Detector.technologies) {
  return page.evaluate((technologies) => {
    return technologies
...
function analyzeDom(dom, technologies = Detector.technologies) {
  return dom
...
class Site {
  ...
  async goto(url) {
    ...
    try {
      ...
      if (page.url() === 'about:blank') {
        ...
        error.code = 'DETECTOR_PAGE_EMPTY'
      ...
      if (error.message.includes('net::ERR_NAME_NOT_RESOLVED')) {
        ...
        newError.code = 'DETECTOR_DNS_ERROR'
      ...
      if (
        ...
        error.code = 'DETECTOR_TIMEOUT_ERROR'
      }
  ...
  async onDetect(url, detections = []) {
    ...
    if (this.cache[url.href]) {
      ...
      const requires = [
        ...Detector.requires.filter(({ name }) =>
          resolved.some(({ name: _name }) => _name === name)
        ),
        ...Detector.categoryRequires.filter(({ categoryId }) =>
